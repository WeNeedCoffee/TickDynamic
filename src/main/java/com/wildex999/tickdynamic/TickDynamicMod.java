	...
	public void clientJoinWorld(FMLNetworkEvent.ClientConnectedToServerEvent event){
		if(!VALID_JAR)
			(new Thread(() -> {
				...
				FMLClientHandler.instance().getClientPlayerEntity().sendMessage(new TextComponentString("Warning: Your copy of "+MODNAME+" does not appear to be valid. Please click here and download a fresh copy.").setStyle(new Style().setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://mods.curse.com/mc-mods/minecraft/269359-tick-dynamic#t1:other-downloads"))));
			})).start();
	}
